...
export default function PhoneKiosk() {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-slate-950 text-slate-100 p-2 sm:p-4 font-sans flex flex-col justify-between max-w-md md:max-w-4xl lg:max-w-7xl mx-auto">
      
      {/* HEADER SECTION */}
      <div className="py-1.5 border-b border-white/10 flex items-center justify-between gap-2 shrink-0">
        <div className="flex items-center gap-2 truncate">
          <h1 className="text-sm sm:text-base md:text-lg lg:text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-300 uppercase tracking-wider whitespace-nowrap truncate">
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg border transition-all duration-500 backdrop-blur-md shadow-sm disabled:opacity-80 ${
              isRefreshing 
                ? 'bg-blue-600/30 border-cyan-400 text-cyan-300 ring-2 ring-blue-500/50' 
                : 'bg-white/5 hover:bg-white/10 border-white/10 text-slate-300 hover:text-white active:scale-95'
            }`}
          >
            <svg
              className={`w-3 h-3 transition-transform duration-1000 ${isRefreshing ? 'animate-spin text-cyan-400' : ''}`}
              fill="none"
              stroke="currentColor"
              strokeWidth="2.2"
              viewBox="0 0 24 24"
      ...
      {errorMsg && (
        <div className="bg-rose-500/10 border border-rose-500/40 text-rose-300 p-2 rounded-lg text-center text-xs my-1 shrink-0 backdrop-blur-md transition-all duration-300">
          ⚠️ {errorMsg}
        </div>
      )}

      {/* MAIN CONTAINER */}
      <div className="flex-1 min-h-0 my-1 grid grid-cols-1 lg:grid-cols-2 gap-3 items-stretch overflow-hidden">

        {/* LEFT COLUMN: ROOM OVERVIEW */}
        <div className="flex flex-col h-full min-h-0">
          <div className="flex items-center justify-between text-[11px] font-bold text-slate-400 uppercase tracking-wider px-1 mb-1 shrink-0">
            <span>Room Overview</span>
            <span className="text-[10px] font-mono text-slate-500">8 Rooms Total</span>
          </div>

          <div className="relative rounded-2xl flex-1 min-h-0 flex">
            <div 
              className={`absolute inset-[-10px] rounded-3xl bg-gradient-to-r from-cyan-500/40 via-blue-600/50 to-indigo-500/40 blur-2xl pointer-events-none transition-all duration-1000 ease-out ${
                isRefreshing 
            ...
            <div className="grid grid-cols-4 gap-2 p-2 rounded-2xl border border-white/10 shadow-2xl backdrop-blur-xl bg-slate-950/80 flex-1 min-h-0 relative z-10">
              {allRooms.slice(0, 8).map((room) => {
                ...
                return (
                  <div
                    key={room.id}
                    className={`p-2 rounded-xl text-center flex flex-col justify-between h-full border transition-all ${
                      isOverbooked
                  ...
                  >
                    <div className="flex items-center justify-between text-[10px] sm:text-[11px] font-black border-b border-white/10 pb-1 shrink-0">
                    ...
                    {checkInGroups && checkInGroups.length > 0 ? (
                      <div className="flex flex-col gap-1 my-auto overflow-y-auto max-h-full py-0.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                        {checkInGroups.map((group, idx) => (
                          <div 
                            key={idx} 
                            className="w-full bg-black/40 rounded-lg p-1.5 text-[8px] sm:text-[8.5px] text-left leading-tight border border-white/5 shadow-inner flex flex-col justify-between shrink-0"
                          >
                            ...
                            <div className="text-[7px] text-slate-400 font-mono mt-0.5 whitespace-nowrap">
                              {formatCheckInTime(group.checked_in_at)}
                            </div>

                            <div className="flex justify-between items-center text-amber-400 font-mono text-[7px] mt-1 whitespace-nowrap">
                              <span>{calculateDuration(group.checked_in_at)}</span>
                              <button
                                onClick={() => handleCheckOut(group.assignment_ids, group.fob_uid)}
                                className="text-rose-400 hover:text-rose-300 hover:underline font-bold ml-1 cursor-pointer"
                              >
                                Out
                              </button>
                            </div>
                      ...
                      <span className="text-emerald-400/80 uppercase tracking-widest text-[8px] font-extrabold my-auto block text-center">
                        Vacant
                      </span>
                    )}
                  </div>
        ...
        <div className="flex flex-col h-full min-h-0">
          ...
          {step === 'SELECT_STAFF' && (
            <div className="relative rounded-2xl h-full flex min-h-0">
              <div 
                className={`absolute inset-[-10px] rounded-3xl bg-gradient-to-r from-cyan-500/40 via-blue-600/50 to-indigo-500/40 blur-2xl pointer-events-none transition-all duration-1000 ease-out ${
                  isRefreshing 
                    ? 'opacity-80 scale-100 animate-pulse' 
              ...
              <div className="flex-1 flex flex-col p-3 rounded-2xl border border-white/10 shadow-2xl backdrop-blur-xl bg-slate-950/80 h-full min-h-0 relative z-10 justify-between">
                ...
                <div className="flex flex-col gap-2 mb-2 shrink-0">
                  <input
                    ...
                    className="w-full bg-slate-900/60 border border-white/10 text-white placeholder-slate-400 text-xs sm:text-sm rounded-xl px-3 py-2focus:outline-none focus:border-blue-500/60 transition shadow-inner"
                  />
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {[
                      ...
                      <button
                        ...
                        className="bg-white/5 hover:bg-white/10 border border-white/10 hover:border-blue-500/40 text-slate-300 hover:text-white text-[10px] font-semibold py-1.5 rounded-lg text-center active:scale-95 transition shadow-sm truncate"
                      >
                        {type.label}
                      </button>
                    ))}
                ...
                {selectedPersons.length > 0 && (
                  <div className="mb-2 p-2 bg-transparent border border-blue-600/40 rounded-xl shrink-0">
                    <div className="text-[9px] uppercase font-bold text-blue-300 mb-1 flex justify-between items-center">
                    ...
                    <div className="flex flex-wrap gap-1 max-h-[50px] overflow-y-auto">
                      {selectedPersons.map((p, idx) => (
                        <span key={idx} className="bg-transparent text-white text-[11px] px-2 py-0.5 rounded-lg flex items-center gap-1">
                ...
                {/* STAFF LIST BUTTONS */}
                <div className="flex-1 min-h-0 grid grid-cols-1 gap-1.5 overflow-y-auto pr-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  {filteredStaff.length > 0 ? (
                    filteredStaff.map((s) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full p-2.5 rounded-xl text-left font-medium text-xs sm:text-sm flex items-center justify-between transition-all shrink-0 ${
                            isAlreadyCheckedIn
                        ...
                        >
                          <span className="flex items-center gap-2 truncate">
                            <span className="p-0.5 rounded bg-white/10 text-xs">👤</span>
                            <span className="truncate">{s.full_name}</span>
                          </span>
                          <span className={`text-[10px] font-mono px-2 py-0.5 shrink-0 ${
                    ...
                    <p className="text-slate-500 text-xs text-center py-4">No staff members found.</p>
                  )}
                </div>
                ...
                <button
                  ...
                  className="mt-2 w-full py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold rounded-xl text-xs sm:text-sm shadow-xl transition active:scale-95 flex items-center justify-center gap-2 shrink-0"
                >
          ...
          {step === 'SELECT_ROOM' && (
            <div className="flex-1 flex flex-col justify-center text-center bg-white/5 p-4 rounded-2xl border border-white/10 backdrop-blur-xl shadow-2xl h-full min-h-0">
              <p className="text-blue-400 font-medium text-sm mb-1 truncate">
                ...
                {allRooms.map((r) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3.5 rounded-xl text-left font-black border shadow-lg active:scale-95 transition flex flex-col justify-between ${
                        isOverCapacity
                    ...
                    >
                      <div className="text-base sm:text-lg">Room {r.room_number}</div>
                      <div className="text-[10px] opacity-90 font-normal mt-1">
              ...
              <button onClick={() => setStep('SELECT_STAFF')} className="mt-3 text-xs text-slate-400 hover:text-white underline transition shrink-0">
          ...
          {step === 'SCAN_KEY' && (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-xl shadow-2xl h-full min-h-0">
          ...
          {step === 'SUCCESS' && (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-emerald-500/10 rounded-2xl border border-emerald-500/40 backdrop-blur-xl shadow-2xl h-full min-h-0">
